...
IMAP4_PORT = 143
IMAP4_SSL_PORT = 993
STARTED, CONNECTED, NONAUTH, AUTH, SELECTED, LOGOUT = (
    "STARTED",
    "CONNECTED",
    "NONAUTH",
    "AUTH",
...
Commands = {
    ...
    "CAPABILITY": Cmd("CAPABILITY", (NONAUTH, AUTH, SELECTED), Exec.async_),
    "CHECK": Cmd("CHECK", (SELECTED,), Exec.async_),
    "CLOSE": Cmd("CLOSE", (SELECTED,), Exec.sync),
    "COMPRESS": Cmd("COMPRESS", (AUTH,), Exec.sync),
    "COPY": Cmd("COPY", (SELECTED,), Exec.async_),
    "CREATE": Cmd("CREATE", (AUTH, SELECTED), Exec.async_),
    "DELETE": Cmd("DELETE", (AUTH, SELECTED), Exec.async_),
    "DELETEACL": Cmd("DELETEACL", (AUTH, SELECTED), Exec.async_),
    "EXAMINE": Cmd("EXAMINE", (AUTH, SELECTED), Exec.sync),
    "EXPUNGE": Cmd("EXPUNGE", (SELECTED,), Exec.async_),
    "FETCH": Cmd("FETCH", (SELECTED,), Exec.async_),
    "GETACL": Cmd("GETACL", (AUTH, SELECTED), Exec.async_),
    "GETQUOTA": Cmd("GETQUOTA", (AUTH, SELECTED), Exec.async_),
    "GETQUOTAROOT": Cmd("GETQUOTAROOT", (AUTH, SELECTED), Exec.async_),
    "ID": Cmd("ID", (NONAUTH, AUTH, LOGOUT, SELECTED), Exec.async_),
    "IDLE": Cmd("IDLE", (SELECTED,), Exec.sync),
    "LIST": Cmd("LIST", (AUTH, SELECTED), Exec.async_),
    "LOGIN": Cmd("LOGIN", (NONAUTH,), Exec.sync),
    "LOGOUT": Cmd("LOGOUT", (NONAUTH, AUTH, LOGOUT, SELECTED), Exec.sync),
    "LSUB": Cmd("LSUB", (AUTH, SELECTED), Exec.async_),
    "MYRIGHTS": Cmd("MYRIGHTS", (AUTH, SELECTED), Exec.async_),
    "NAMESPACE": Cmd("NAMESPACE", (AUTH, SELECTED), Exec.async_),
    "NOOP": Cmd("NOOP", (NONAUTH, AUTH, SELECTED), Exec.async_),
    "RENAME": Cmd("RENAME", (AUTH, SELECTED), Exec.async_),
    "SEARCH": Cmd("SEARCH", (SELECTED,), Exec.async_),
    ...
    "SORT": Cmd("SORT", (SELECTED,), Exec.async_),
    "STARTTLS": Cmd("STARTTLS", (NONAUTH,), Exec.sync),
    "STATUS": Cmd("STATUS", (AUTH, SELECTED), Exec.async_),
    "STORE": Cmd("STORE", (SELECTED,), Exec.async_),
    "SUBSCRIBE": Cmd("SUBSCRIBE", (AUTH, SELECTED), Exec.sync),
    "THREAD": Cmd("THREAD", (SELECTED,), Exec.async_),
    "UID": Cmd("UID", (SELECTED,), Exec.async_),
...
class Command(object):
    ...
    def wait_literal_data(self):
        return (
            self._expected_size != 0 and len(self._literal_data) != self._expected_size
        )
...
class IMAP4ClientProtocol(asyncio.Protocol):
    ...
    def data_received(self, d):
        ...
        try:
            self._handle_responses(
                self.incomplete_line + d, self._handle_line, self.current_command
            )
            self.incomplete_line = b""
            self.current_command = None
            ...
            log.debug(
                "Incomplete line, storing partial : %s" % incomplete_read.partial[:100]
            )
    ...
    def execute(self, command):
        ...
        if Commands.get(command.name).exec == Exec.sync:
            ...
            if self.pending_async_commands.get(command.untagged_resp_name) is not None:
                yield from self.pending_async_commands[
                    command.untagged_resp_name
                ].wait()
    ...
    def logout(self):
        response = (
            yield from self.execute(Command("LOGOUT", self.new_tag(), loop=self.loop))
        )
        if "OK" == response.result:
            self.state = LOGOUT
        return response
    ...
    def close(self):
        response = yield from self.execute(
            Command("CLOSE", self.new_tag(), loop=self.loop)
        )
    ...
    def idle(self):
        ...
        return (
            yield from self.execute(Command("IDLE", self.new_tag(), loop=self.loop))
        )
    ...
    def expunge(self):
        return (
            yield from self.execute(Command("EXPUNGE", self.new_tag(), loop=self.loop))
        )
    ...
    def uid(self, command, *criteria):
        ...
        if command.upper() == "COPY":
            ...
            raise Abort(
                "command UID only possible with COPY, FETCH or STORE (was %s)"
                % command.upper()
            )
    ...
    def capability(self):
        response = yield from self.execute(
            Command("CAPABILITY", self.new_tag(), loop=self.loop)
        )
    ...
    def append(self, message_bytes, mailbox="INBOX", flags=None, date=None):
        ...
        return (
            yield from self.execute(
                Command("APPEND", self.new_tag(), *args, loop=self.loop)
            )
        )
    ...
    def simple_command(self, name, *args):
        if name not in self.simple_commands:
            raise NotImplementedError(
                "simple command only available for %s" % self.simple_commands
            )
        return (
            yield from self.execute(
                Command(name, self.new_tag(), *args, loop=self.loop)
            )
        )
    ...
    def wait_async_pending_commands(self):
        yield from asyncio.wait(
            [
                asyncio.ensure_future(cmd.wait())
                for cmd in self.pending_async_commands.values()
            ]
        )
    ...
    def has_pending_idle_command(self):
        return (
            self.pending_sync_command is not None
            and self.pending_sync_command.name == "IDLE"
        )

    def _untagged_response(self, line):
        if self.pending_sync_command is not None:
            if self.has_pending_idle_command():
                asyncio.ensure_future(self.idle_queue.put(line))
                return
            else:
    ...
    def _response_done(self, line):
        ...
        if self.pending_sync_command is not None:
            if self.pending_sync_command.tag != tag:
                raise Abort(
                    "unexpected tagged response with pending sync command (%s) response: %s"
            ...
            if len(cmds) == 0:
                ...
                raise Error(
                    "inconsistent state : two commands have the same tag (%s)" % cmds
                )
            command = cmds.pop()
    ...
    def _find_pending_async_cmd_by_tag(self, tag):
        return [
            c
            for c in self.pending_async_commands.values()
...
class IMAP4(object):
    ...
    def login(self, user, password):
        return (
            yield from asyncio.wait_for(
                self.protocol.login(user, password), self.timeout
            )
        )
    ...
    def select(self, mailbox="INBOX"):
        return (
            yield from asyncio.wait_for(self.protocol.select(mailbox), self.timeout)
        )

    def examine(self, mailbox="INBOX"):
        return (
            yield from asyncio.wait_for(self.protocol.examine(mailbox), self.timeout)
        )
    ...
    def search(self, *criteria, charset="utf-8"):
        return (
            yield from asyncio.wait_for(
                self.protocol.search(*criteria, charset=charset), self.timeout
            )
        )
    ...
    def uid_search(self, *criteria, charset="utf-8"):
        return (
            yield from asyncio.wait_for(
                self.protocol.search(*criteria, by_uid=True, charset=charset),
                self.timeout,
            )
        )
    ...
    def uid(self, command, *criteria):
        return (
            yield from asyncio.wait_for(
                self.protocol.uid(command, *criteria), self.timeout
            )
        )
    ...
    def store(self, *criteria):
        return (
            yield from asyncio.wait_for(self.protocol.store(*criteria), self.timeout)
        )

    @asyncio.coroutine
    def copy(self, *criteria):
        return (
            yield from asyncio.wait_for(self.protocol.copy(*criteria), self.timeout)
        )
    ...
    def fetch(self, message_set, message_parts):
        return (
            yield from self.protocol.fetch(
                message_set, message_parts, timeout=self.timeout
            )
        )
    ...
    def noop(self):
        return (
            yield from asyncio.wait_for(
                self.protocol.simple_command("NOOP"), self.timeout
            )
        )
    ...
    def check(self):
        return (
            yield from asyncio.wait_for(
                self.protocol.simple_command("CHECK"), self.timeout
            )
        )
    ...
    def status(self, mailbox, names):
        return (
            yield from asyncio.wait_for(
                self.protocol.simple_command("STATUS", mailbox, names), self.timeout
            )
        )
    ...
    def subscribe(self, mailbox):
        return (
            yield from asyncio.wait_for(
                self.protocol.simple_command("SUBSCRIBE", mailbox), self.timeout
            )
        )
    ...
    def unsubscribe(self, mailbox):
        return (
            yield from asyncio.wait_for(
                self.protocol.simple_command("UNSUBSCRIBE", mailbox), self.timeout
            )
        )
    ...
    def lsub(self, reference_name, mailbox_name):
        return (
            yield from asyncio.wait_for(
                self.protocol.simple_command("LSUB", reference_name, mailbox_name),
                self.timeout,
            )
        )
    ...
    def create(self, mailbox_name):
        return (
            yield from asyncio.wait_for(
                self.protocol.simple_command("CREATE", mailbox_name), self.timeout
            )
        )
    ...
    def delete(self, mailbox_name):
        return (
            yield from asyncio.wait_for(
                self.protocol.simple_command("DELETE", mailbox_name), self.timeout
            )
        )
    ...
    def rename(self, old_mailbox_name, new_mailbox_name):
        return (
            yield from asyncio.wait_for(
                self.protocol.simple_command(
                    "RENAME", old_mailbox_name, new_mailbox_name
                ),
                self.timeout,
            )
        )
    ...
    def list(self, reference_name, mailbox_pattern):
        return (
            yield from asyncio.wait_for(
                self.protocol.simple_command("LIST", reference_name, mailbox_pattern),
                self.timeout,
            )
        )
    ...
    def append(self, message_bytes, mailbox="INBOX", flags=None, date=None):
        return (
            yield from asyncio.wait_for(
                self.protocol.append(message_bytes, mailbox, flags, date), self.timeout
            )
        )
